...
@RestController
@RequestMapping(path = "dronebase/pakket", produces = { MediaType.APPLICATION_JSON_VALUE, MediaType.TEXT_PLAIN_VALUE })
...
public class PakketEndpoint {
	...
	public ResponseEntity<Pakket> apiCreate(@RequestBody Pakket pakket) {
		...
		return new ResponseEntity<Pakket>(pakketService.save(pakket), HttpStatus.OK);
	}

	// Retrieve
	@GetMapping
	public ResponseEntity<Iterable<Pakket>> apiGetAll() {
		return new ResponseEntity<Iterable<Pakket>>(pakketService.findAll(), HttpStatus.OK);
	}

	@GetMapping(path = "{id}")
	public ResponseEntity<Optional<Pakket>> apiGetById(@PathVariable long id) {
		Optional<Pakket> pakket = pakketService.findById(id);
		return new ResponseEntity<>(pakket, pakket.isPresent() ? HttpStatus.OK : HttpStatus.NOT_FOUND);
	}

	// Update
	@PutMapping(path = "{id}")
	public ResponseEntity<Pakket> apiUpdate(@PathVariable("id") long id, @RequestBody Pakket pakket) {
		...
		return new ResponseEntity<>(pakketService.save(pakket), HttpStatus.OK);
	}
